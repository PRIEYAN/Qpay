 ...
 */
export const WALLETCONNECT_PROJECT_ID = 'c9332736763894f0c4bb80b7929c1cc5';
...
/** Where to get the missing project id — one copy, shared by every surface that reports it. */
export const WALLETCONNECT_HINT =
  'Create a free project at https://cloud.reown.com and paste its Project ID into WALLETCONNECT_PROJECT_ID in src/config/network.ts';

/**
 * The missing-project-id message, in the same summary + indented-checklist
 * shape `assertConfigured()` produces, so screens can render it with the same
 * "setup isn't finished" treatment instead of a generic connect prompt.
 */
export function walletConnectSetupMessage(): string {
  return `Qpay is not fully configured yet. Missing:\n  - WALLETCONNECT_PROJECT_ID: ${WALLETCONNECT_HINT}`;
}
